...
namespace VideoAudioMerge
{
    static class OpenFolder
    {
        ...
        public static void FilesOrFolders(string parentDirectory, IEnumerable<string> filenames)
        {
            if (!Directory.Exists(parentDirectory)) throw new DirectoryNotFoundException();
            if (filenames == null) throw new ArgumentNullException("filenames");

            // Filter invalid files and folders
            filenames = filenames.
                    Select(str => str.Trim().Trim('\\'))
                    .Select(str => Path.GetFileName(str))
                    .Where(str => File.Exists(parentDirectory + '\\' + str) ||
                        Directory.Exists(parentDirectory + '\\' + str));
        ...
        public static void FilesOrFolders(string parentDirectory, params string[] paths)
        {
            FilesOrFolders(parentDirectory, (IEnumerable<string>)paths);
        }
    }
}
